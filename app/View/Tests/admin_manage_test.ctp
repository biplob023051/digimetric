...
<div class="row">
    <div class="col-md-12"> 
        ...
        <ul class="page-breadcrumb breadcrumb">
            <li class="btn-group"> <a href="<?php echo ADMIN_BASEURL ?>tests/add_test" class="btn blue dropdown-toggle"> <span> Add test </span> </a> </li>
            <li> <i class="fa fa-home"></i> <a href="<?php echo ADMIN_BASEURL; ?>admins/dashboard"> Home </a> <i class="fa fa-angle-right"></i> </li>
            <li> <a href="javascript:void(0);"> Manage Tests </a> </li>
        </ul>
    ...
    <div class="col-md-12">
        <div class="tabbable tabbable-custom tabbable-full-width">
            <ul class="nav nav-tabs">
                <li class="active"> <a data-toggle="tab" href="#tab_1_5"> Manage Tests </a> </li>
            </ul>
...
//                                        isset($v['tag']['BlogTag']) ? $tag_array = $v['tag']['BlogTag'] : '';
                                        ?>
                                        <tr>
                                            <td><?php echo $v["Test"]["id"]; ?></td>
                                            <td><?php echo $v["Test"]["title"]; ?></td>
                                            <td><?php echo $v["Test"]["description"]; ?></td>
                                            <td><?php echo $v["Area"]["title"]; ?></td>
                                            <td><?php echo $v["Category"]["title"]; ?></td>
                                            <td><?php echo $v["SubCategory"]["title"]; ?></td>
                                            <td>
                                                <img alt="Logo" src="<?php echo BASEURL ?>/uploads/test_logo/<?php echo $v['Test']['logo']; ?>" height="30" width="30"/>
                                                <?php //echo $v["Test"]["logo"]; ?>
                                            </td>
                                            <td><?php echo $v["Difficulty"]["title"]; ?></td>
                                            <td><?php echo $v["Test"]["no_of_questions"]; ?></td>
                                            <td>
                                                <?php echo!empty($v['Test']['duration_hour']) ? $v['Test']['duration_hour'] . " Hour(s)" : ''; ?>
                                                <?php echo!empty($v['Test']['duration_mins']) ? $v['Test']['duration_mins'] . " Min(s)" : ''; ?>
                                                <?php echo!empty($v['Test']['duration_secs']) ? $v['Test']['duration_secs'] . " Sec(s)" : ''; ?>
                                            </td>


                                            <td>
                                                <?php if ($v["Test"]["status"] == 1) { ?>
                                                    <span class="label label-sm label-success"> <a href="<?php echo ADMIN_BASEURL; ?>tests/enable_disable_test/<?php echo $v["Test"]["id"]; ?>/0" onclick="return enable_disable_test('disable');">Inactivate</a> </span>
                                                <?php } else { ?>
                                                    <span class="label label-sm label-success"> 
                                                        <a href="<?php echo ADMIN_BASEURL; ?>tests/enable_disable_test/<?php echo $v["Test"]["id"]; ?>/1" onclick="return enable_disable_test('enable');">
                                                            Activate
                                                        </a>
                                                    </span>
                                                <?php } ?>
                                            ...
                                            <td>
                                                <form method="get" action="<?php echo ADMIN_BASEURL; ?>test_questions/manage_test_questions">
                                                    <?php if(!empty($v["Test"]["versions"])){
                                                        $no_versions = $v["Test"]["versions"]; ?>
                                                    <select name="version">
                                                        <?php for ($i = 1; $i <= $no_versions; $i++) { ?>
                                                            <option  value="<?php echo $i; ?>"><?php echo $i; ?></option>
                                                        <?php } ?>
                                                    </select>
                                                    <input type="hidden" name="test_id" value="<?php echo $v["Test"]["id"]; ?>"/>
                                                    <input type="submit" name="manage_versions" value="Manage"/>
                                                    <?php }else{ echo "Versions not found"; } ?>
                                            ...
                                            <td>
                                                ...
                                                <a class="can_delete" href=" <?php echo ADMIN_BASEURL; ?>tests/delete_test/<?php echo $v["Test"]["id"]; ?>"  onclick="return delete_fun();"><i class="can_delete fa fa-times fonta"></i></a> 
                                                <a class="" href="<?php echo ADMIN_BASEURL; ?>tests/edit_test/<?php echo $v["Test"]["id"]; ?>"><i class="fa fa-edit fonta"></i></a>
                                            </td>
                                        </tr>
                                        <?php
                                    }
...
<script>
    ...
    function enable_disable_test(item) {
        if (confirm("Are you sure you want to " + item + " the test?")) {
            return true;
        }
        else {
            return false;
